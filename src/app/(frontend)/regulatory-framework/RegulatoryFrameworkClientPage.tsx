...
interface Document { title: string; file: { url: string; }; }
interface Section { id: string; title: string; description: string; documents: Document[]; }
interface PageProps { sidebarLinks: SidebarLink[]; sections: Section[]; }
...
// Компоненты-обертки
const DocumentLink = ({ title, href }: { title: string, href: string }) => ( <Link href={href} target="_blank" className="flex items-center w-full sm:w-auto max-w-sm gap-6 p-3 rounded-lg bg-[#4aa30a]/[0.08] hover:bg-[#4aa30a]/[0.15] transition-colors"><PDFIcon /><p className="text-sm font-medium text-black">{title}</p></Link> );
const AccordionSection = ({ title, isOpen, onClick, children }: { title: string; isOpen: boolean; onClick: () => void; children: React.ReactNode; }) => ( <div className="flex flex-col border-b border-black/10 pb-8"><div onClick={onClick} className="flex items-center justify-between w-full cursor-pointer"><p className="text-2xl md:text-[28px] font-semibold text-black pr-4">{title}</p><button aria-label={isOpen ? "Свернуть" : "Развернуть"}>{isOpen ? <MinusIcon /> : <PlusIcon />}</button></div><div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-[1000px] pt-8' : 'max-h-0'}`}>{children}</div></div> );

export default function RegulatoryFrameworkClientPage({ sidebarLinks, sections }: PageProps) {
  const t = useTranslations('RegulatoryFrameworkPage');
  const pathname = usePathname();
  const [openSectionId, setOpenSectionId] = useState<string | null>(sections[0]?.id || null);

  const handleToggleSection = (id: string) => { setOpenSectionId(openSectionId === id ? null : id); };
    ...
    <section className="bg-white">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex items-center gap-2 text-sm pt-8 mb-8">
          ...
          <BreadcrumbSeparator />
          <span className="text-black/80">{t('breadcrumb.about')}</span>
        </div>
            ...
            <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-8">
                {sections.map(section => (
                  <AccordionSection key={section.id} title={section.title} isOpen={openSectionId === section.id} onClick={() => handleToggleSection(section.id)}>
                    <div className="flex flex-col gap-8">
