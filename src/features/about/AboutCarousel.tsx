...
const aboutIcons = ['/svg/math.svg', '/svg/cpu.svg', '/svg/abc.svg', '/svg/spiral.svg']
          ...
          {aboutIcons.map((icon, idx) => (
            <CarouselItem key={idx} className="md:basis-1/2 lg:basis-1/4 pl-2 md:pl-4 ">
              <div className="px-4 py-10 bg-[#ECFFE7] h-full rounded-2xl flex items-center justify-center flex-col">
                ...
                <h3 className="text-black mb-2 text-center">{t(`aboutCarousel.bullets.${idx}.title`)}</h3>
                <p className="text-black/60 text-center">{t(`aboutCarousel.bullets.${idx}.description`)}</p>
              </div>
            </CarouselItem>
          ))}
        </CarouselContent>
